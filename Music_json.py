...
class Music(commands.Cog, name='Music'):
    ...
    def play_song(self, voice_id):
        with open('queue.json', 'r') as f:
            queue = json.load(f)
        info = get_stream_url(queue[str(voice_id)][1]['url'])
        source = create_ytdl_source(info)
        text_channel = self.client.get_channel(int(
            queue[str(voice_id)][0]['text_channel']
        ))
            ...
            if voice != None:
                if voice.channel != channel:
                    if len(voice.channel.members) == 1:
                        ...
                        await voice.move_to(channel)
                        queue[str(voice.channel.id)] = [
                            {
                                'loop': 'off',
                                "text_channel": str(text_channel.id),
                            },
                        ]
                        queue[str(voice.channel.id)].append(
                            {
                                'url': info[0],
                                'title': info[1]
                            }
                        ...
                        await ctx.channel.purge(limit=1)
                        self.play_song(voice.channel.id)
                    elif voice.is_playing() or voice.is_paused():
                        await ctx.channel.purge(limit=1)
                        await ctx.send(
                            embed=create_embed(
